...
static u64 MOS_ALIGNED(8) LastCycleCount = 0;
...
static s32 ThreadExit(s32 rtn_val) {
    ...
    asm volatile ( "dmb" );
    if (MosIsOnList(&RunningThread->tmr_e))
        MosRemoveFromList(&RunningThread->tmr_e);
    MosList * elm_save;
    for (MosList * elm = RunningThread->stop_q.next;
        ...
        if (MosIsOnList(&thd->tmr_e))
            MosRemoveFromList(&thd->tmr_e);
...
static void InitThread(Thread * thd, MosThreadPriority pri,
    ...
    *((u32 *)sp) = STACK_FILL_VALUE;
    ...
    // Fill lower stack including canary value on bottom
    u32 * fill = (u32 *)sf - 1;
...
static u32 MOS_USED Scheduler(u32 sp) {
    ...
    u64 cycle_count = MosGetCycleCount();
    ...
    for (MosThreadPriority pri = 0; pri < MOS_MAX_THREAD_PRIORITIES; pri++) {
        if (!MosIsListEmpty(&RunQueues[pri])) {
            run_thd = container_of(RunQueues[pri].next, Thread, run_e);
            break;
        }
    }
    if (run_thd) {
